...
describe('ProductCategory e2e test', () => {
  ...
  it('should create an instance of ProductCategory', () => {
    ...
    cy.intercept('GET', '/services/productorder/api/product-categories*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('product-category');
  ...
  it('should delete last instance of ProductCategory', () => {
    ...
    cy.wait('@entitiesRequest').then(({ request, response }) => {
      ...
      if (startingEntitiesCount > 0) {
        ...
        cy.intercept('GET', '/services/productorder/api/product-categories*').as('entitiesRequestAfterDelete');
        cy.visit('/');
        cy.clickOnEntityMenuItem('product-category');
