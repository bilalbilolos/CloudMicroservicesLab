...
describe('OrderItem e2e test', () => {
  ...
  it('should create an instance of OrderItem', () => {
    ...
    cy.intercept('GET', '/services/productorder/api/order-items*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('order-item');
  ...
  it('should delete last instance of OrderItem', () => {
    ...
    cy.wait('@entitiesRequest').then(({ request, response }) => {
      ...
      if (startingEntitiesCount > 0) {
        ...
        cy.intercept('GET', '/services/productorder/api/order-items*').as('entitiesRequestAfterDelete');
        cy.visit('/');
        cy.clickOnEntityMenuItem('order-item');
