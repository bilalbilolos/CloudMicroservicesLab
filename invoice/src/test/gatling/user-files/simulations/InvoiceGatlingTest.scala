...
class InvoiceGatlingTest extends Simulation {
    ...
    val baseURL = Option(System.getProperty("baseURL")) getOrElse """http://104.199.45.53:8080"""
    ...
    setUp(
        users.inject(rampUsers(Integer.getInteger("users", 1000)) during (Integer.getInteger("ramp", 2) minutes))
    ).protocols(httpConf)
}
